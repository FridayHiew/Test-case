...
export default function SettingsPanel({ config, onConfigChange, llmClient }: SettingsPanelProps) {
  ...
  // Programmatic Baseline Templates config state
  ...
  const handleToggleTemplate = (id: string) => {
    ...
    saveConfig({ programmaticTemplates: updated });
  };
  ...
  const handleResetTemplates = () => {
    if (window.confirm('Are you sure you want to restore the programmatic test case baseline to factory defaults? This will erase all custom configurations.')) {
      ...
      saveConfig({ programmaticTemplates: DEFAULT_BASE_TEMPLATES });
      setEditingId(null);
    }
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200/80 p-6 space-y-6">
      ...
      {/* Baseline Case Structure Configuration */}
      <div className="pt-4 border-t border-slate-100 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2 text-slate-700">
            <Database className="w-4 h-4 text-blue-500" />
            <span className="text-sm font-semibold text-slate-900">Baseline Case Structure Templates</span>
          </div>
          <button
            type="button"
            onClick={handleResetTemplates}
            className="inline-flex items-center text-[10px] sm:text-xs font-semibold text-rose-600 hover:text-rose-800 transition py-1 px-2.5 bg-rose-50 border border-rose-100 hover:border-rose-200 rounded-lg"
          >
        ...
        <p className="text-xs text-slate-500 leading-relaxed">
          Customize the high-performance base test cases generated dynamically by code before invoking AI models. You can toggle templates, add custom checks, or rewrite structural steps.
        </p>
        ...
        <div className="p-3 bg-slate-50 rounded-lg border border-slate-100 text-[11px] text-slate-600 space-y-1">
          <span className="font-bold text-slate-800">💡 Available Placeholders:</span>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 font-mono text-[10px]">
            <div><code>{`{feature_name}`}</code> - Feature name</div>
            <div><code>{`{feature_id}`}</code> - Feature spec ID</div>
            <div><code>{`{required_inputs}`}</code> - Required field list</div>
            <div><code>{`{bounds_steps}`}</code> - Dynamic boundary steps</div>
            <div><code>{`{output_names}`}</code> - Outputs contract</div>
            <div><code>{`{dependencies}`}</code> - Prerequisites</div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {templates.map((template) => {
            ...
            return (
              <div key={template.id} className={`p-4 border rounded-xl space-y-3 transition ${
                isEditing ? 'border-blue-300 bg-blue-50/20' : 'border-slate-150 bg-slate-50/50 hover:bg-slate-50'
              }`}>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-center space-x-2.5">
                    <input
                      ...
                      className="w-4 h-4 text-blue-600 focus:ring-blue-500 border-slate-350 rounded cursor-pointer"
                      title={template.enabled ? 'Click to Disable' : 'Click to Enable'}
                    />
                    <div>
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold uppercase mr-2 ${
                      ...
                      <span className="font-mono text-[10px] text-slate-400 font-bold mr-1">[{template.id}]</span>
                    </div>
                  </div>
                  ...
                  {!isEditing && (
                    <div className="flex items-center space-x-1.5 self-end sm:self-auto">
                      <button
                        ...
                        className="inline-flex items-center px-2 py-1 text-[11px] font-semibold text-slate-600 hover:text-slate-800 bg-white hover:bg-slate-100 border border-slate-200 rounded-lg transition"
                      >
                      ...
                      <button
                        ...
                        className="inline-flex items-center px-2 py-1 text-[11px] font-semibold text-rose-600 hover:text-rose-800 bg-white hover:bg-rose-50 border border-slate-200 hover:border-rose-200 rounded-lg transition"
                      >
                ...
                {/* Edit Form */}
                {isEditing ? (
                  <div className="space-y-3.5 bg-white p-3.5 border border-slate-200 rounded-lg text-xs">
                    <div>
                      <label className="block text-[10px] uppercase font-bold text-slate-400 mb-1">Template Title Pattern</label>
                      <input
                        type="text"
                        value={editTitle}
                        onChange={(e) => setEditTitle(e.target.value)}
                        className="w-full px-2.5 py-1.5 border border-slate-200 rounded bg-slate-50 focus:bg-white focus:ring-1 focus:ring-blue-500 outline-none font-sans font-semibold text-slate-800"
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[10px] uppercase font-bold text-slate-400 mb-1">Test Type Category</label>
                        <select
                          value={editType}
                          onChange={(e) => setEditType(e.target.value as any)}
                          className="w-full px-2.5 py-1.5 border border-slate-200 rounded bg-slate-50 focus:bg-white focus:ring-1 focus:ring-blue-500 outline-none"
                        >
                    ...
                    <div>
                      <label className="block text-[10px] uppercase font-bold text-slate-400 mb-1">Preconditions (One per line)</label>
                      <textarea
                        value={editPreconditions}
                        onChange={(e) => setEditPreconditions(e.target.value)}
                        rows={2}
                        className="w-full p-2.5 border border-slate-200 rounded bg-slate-50 focus:bg-white focus:ring-1 focus:ring-blue-500 outline-none font-mono text-[11px] leading-normal"
                        placeholder="e.g. Subsystem &quot;{feature_name}&quot; is loaded."
                      />
                    </div>

                    <div>
                      <label className="block text-[10px] uppercase font-bold text-slate-400 mb-1">Steps (One per line)</label>
                      <textarea
                        value={editSteps}
                        onChange={(e) => setEditSteps(e.target.value)}
                        rows={3}
                        className="w-full p-2.5 border border-slate-200 rounded bg-slate-50 focus:bg-white focus:ring-1 focus:ring-blue-500 outline-none font-mono text-[11px] leading-normal"
                        placeholder="e.g. 1. Fill in {required_inputs}"
                      />
                    </div>

                    <div>
                      <label className="block text-[10px] uppercase font-bold text-slate-400 mb-1">Expected Outcome Pattern</label>
                      <textarea
                        value={editExpected}
                        onChange={(e) => setEditExpected(e.target.value)}
                        rows={2}
                        className="w-full p-2.5 border border-slate-200 rounded bg-slate-50 focus:bg-white focus:ring-1 focus:ring-blue-500 outline-none font-mono text-[11px] leading-normal"
                        placeholder="e.g. Request executes successfully."
                      />
                    </div>
                    ...
                    <div className="flex justify-end space-x-2 pt-1">
                      <button
                        ...
                        className="inline-flex items-center px-3 py-1.5 border border-slate-200 text-slate-600 bg-white hover:bg-slate-50 rounded-lg text-xs font-semibold transition"
                      >
                      ...
                      <button
                        ...
                        className="inline-flex items-center px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition"
                      >
                  ...
                  <div className="space-y-2.5 text-xs text-slate-600 pl-6 border-l-2 border-slate-200/60 leading-relaxed font-sans">
                    <div className="font-semibold text-slate-800 text-[13px]">{template.title}</div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[11px] bg-white/70 p-2.5 rounded-lg border border-slate-100">
                      <div>
                        <span className="text-[9px] uppercase font-bold text-slate-400 block mb-0.5">Preconditions:</span>
                        <ul className="list-disc pl-4 space-y-0.5">
                          {template.preconditions.map((p, idx) => <li key={idx} className="truncate">{p}</li>)}
                        </ul>
                      </div>
                      <div>
                        <span className="text-[9px] uppercase font-bold text-slate-400 block mb-0.5">Expected Outcome:</span>
                        <span className="text-slate-600 block line-clamp-2">{template.expected}</span>
                      </div>
                    </div>
                  </div>
                )}
        ...
        <button
          ...
          className="w-full flex items-center justify-center py-2 px-4 border border-dashed border-slate-300 hover:border-blue-400 rounded-xl text-xs font-bold text-slate-600 hover:text-blue-600 bg-white hover:bg-blue-50/30 transition"
        >
